...
2. 起動時の初期化処理を実装:
   - `loadThreads()` で LocalStorage の `ask-ai:threads` からスレッド一覧を復元
   - `loadCurrentThreadId()` で LocalStorage の `ask-ai:current-thread` から選択中スレッドIDを復元
   - currentThreadId が threads 内に存在しない場合（ID不整合）、threads の先頭スレッドにフォールバックし、`ask-ai:current-thread` を即時保存して永続化
   - threads が空（0件）の場合、デフォルトスレッドを新規作成し、`ask-ai:threads` と `ask-ai:current-thread` を即時保存して永続化
3. スレッド作成関数を実装:
