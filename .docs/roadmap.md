...
### マイルストーン達成条件
- [x] 新しいスレッドを作成して会話を開始できる
- [x] スレッド一覧から過去の会話を選択して再開できる
- [x] 不要なスレッドを削除できる
- [x] トークン上限を超える長い会話でもエラーにならない
...
| タスクID | 状態 | 依存タスク | 対応要件 | 概要 |
|----------|------|-----------|----------|------|
| T-M5-1 | ✅ | - | FR-013, FR-017 | 型定義の拡張とカスタムコマンド永続化層の実装 |
| T-M5-2 | ✅ | T-M5-1 | FR-013 | デフォルトシステムプロンプト（Preferences）の実装 |
| T-M5-3 | ✅ | T-M5-1 | FR-014 | カスタムコマンド作成コマンドの実装 |
| T-M5-4 | ✅ | T-M5-3 | FR-015 | カスタムコマンド一覧管理コマンドの実装 |
| T-M5-5 | ✅ | T-M5-2, T-M5-3 | FR-016, FR-017, FR-018 | チャット画面へのカスタムコマンド切替統合 |

### マイルストーン達成条件
- [x] Preferences でデフォルトシステムプロンプトを設定でき、全会話に反映される
- [x] カスタムコマンドを作成・編集・削除できる
- [x] カスタムコマンド一覧から会話を開始できる
- [x] チャット画面のドロップダウンでカスタムコマンドを切り替えられる
- [x] カスタムコマンドのモデル設定が Preferences より優先される
- [x] 削除済みカスタムコマンドの場合にデフォルトにフォールバックする
- [x] 既存の Thread データ（customCommandId なし）が問題なく動作する
