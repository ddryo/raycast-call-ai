...
### マイルストーン達成条件
- [ ] `npm run dev` でエラーなくビルドが通る
- [ ] 型定義・API通信層・永続化層が個別モジュールとして存在する
- [ ] Preferences に API キーを設定できる
...
### マイルストーン達成条件
- [ ] SearchBar からメッセージを送信し、AI の応答が表示される
- [ ] 会話履歴が List に時系列で表示される
- [ ] 選択メッセージの内容が Detail（markdown）に表示される
- [ ] 拡張機能を再起動しても会話が復元される
- [ ] 複数行テキストを Form から入力・送信できる
...
### マイルストーン達成条件
- [ ] API エラー時に種別に応じた適切な Toast メッセージが表示される
- [ ] 送信中は入力がロックされ、ローディング表示がされる
- [ ] 会話履歴を全クリアできる
- [ ] 選択メッセージをクリップボードにコピーできる
- [ ] Phase 1 の全機能が安定して動作する
